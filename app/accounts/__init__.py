...
def admin(template="admin.html"):
    ...
    if request.method == "GET":
        ...
        return render_template(template, userlist=result, pages=pages, page=page, maxpage=maxpage, limit=LIMIT, count=count, search_name = request.args["name"] if "name" in request.args else "")
    elif request.method == "POST":
        param = None
        val = None
