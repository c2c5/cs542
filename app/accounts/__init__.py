...
def admin():
    ...
    if request.method == "GET":
        ...
        return render_template('admin.html', userlist=result, pages=pages, page=page, maxpage=maxpage, limit=LIMIT,
                               count=count, search_name=request.args["name"] if "name" in request.args else "")
    elif request.method == "POST":
        param = None
        val = None
...
def edit(id):
    ...
    if (request.method == "GET"):
        ...
        if ("delete" in request.form):
            ...
            if (request.form["password"] == "" or request.form["confirmpassword"] == "" or request.form[
            ...
            with db.cursor() as cursor:
                ...
                if (("admin" in current_user_roles() and current_u["userid"] != int(id)) or bcrypt.checkpw(
                    ...
                    with db.cursor() as cursor:
                        ...
                        cursor.execute(changepw,
                                       (bcrypt.hashpw(request.form["password"].encode('utf-8'), bcrypt.gensalt()), id))
                    db.commit()
